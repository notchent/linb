    ...
    Instance:{
        ...
        */
        _insertRowsToDom:function(profile, arr, pid, base, before){
            ...
            //get column width
            var ws=[], hw=profile.getSubNode(profile.keys.HCELL0).width();
            profile.properties.header.each(function(v){
                ws.push(profile.getSubNode(profile.keys.HCELL, profile.header_ItemIdMapSerialId[v.id]).width());
            });
            ...
            if(!base){
                ...
                if(pid){
                    obj = profile.getSubNode(profile.keys.SUB, pid);
                }else{
                    obj = profile.getSubNode(profile.keys.BODY);
                }
                if(before)
                    obj.addFirst(nodes);
                else
                ...
                //
                obj = profile.getSubNode(profile.keys.CELLS, base);
                if(before)
                    obj.addPre(nodes);
                else{
                    nodes.get().reverse();
        ...
        refreshHeader:function(header){
            ...
            profile.getSubNode(profile.keys.HCELL, true).remove();
            profile.getSubNode(profile.keys.HCELLS).addLast(nodes);
        ...
        */
        removeRows:function(ids){
            ...
            ids.each(function(id){
                ...
                if(row = profile.row_SerialIdMapItem[id]){
                    ...
                    if(temp= row._parent?(temp=profile.row_SerialIdMapItem[row._parent])?temp.sub:null:profile.properties.row)
                    ...
                    nodes.push(profile.getSubNode(profile.keys.CELLS, id));
                    //for sub delete
                    if(row.sub){
                        var arr=[];
        ...
        removeAllRows:function(){
            ...
            profile.getSubNode(profile.keys.BODY).empty();
            profile.getSubNode(profile.keys.SCROLL).scrollTop(0).scrollLeft(0);
        ...
        */
        ...
        updateColumn:function(colId, hashValue){
            ...
            _.each(hashValue,function(o,i){
                ...
                pro.getSubNode(pro.keys.CELLA, cells[i]).html(o, false);
            });
        ...
        scrollTo:function(pos){
            var profile=this.get(0), scroll=profile.getSubNode(profile.keys.SCROLL);
        ...
        b.CELLA.onKeydown = b.CELL0A.onKeydown = b.HCELLA.onKeydown = b.HCELL0A.onKeydown =function(profile, e, src){
            ...
            case 'down':
                ...
                if(!profile.$allrowscache){
                    var all=[];
                    all.push(profile.getSubNode(profile.keys.HCELLS).get(0));
                    all.insert(profile.getSubNode(profile.keys.CELLS,true).get(),-1);
                    //filter dispaly==none
    ...
    Static:{
        ...
        Behaviors:{'default':{
            ...
            SCROLL:{
                onScroll:function(profile, e, src){
                    ...
                    if(profile.oldPos!=pos){
                        profile.getSubNode(profile.keys.HEADER).left(-pos);
                        profile.oldPos=pos;
                    }
                }
            },
            ...
            HCELLHANDLER:{
                onMousedown:function(profile, e, src){
                    ...
                    scroll = profile.getSubNode(profile.keys.SCROLL),
                ...
                onDragbegin:function(profile, e, src){
                    ...
                    .setStyle({
                        height:profile.getSubNode(profile.key).height()+'px',
                        width:'4px',
                        backgroundColor:'#ddd',
                        cursor:'e-resize'
                    });
                ...
                onDrag:function(profile, e, src){
                    ...
                    if(d.left<=d.limit_left || d.left>=d.limit_right)b=true;
                    if(b){
                        if(!profile._limited){
                ...
                onDragend:function(profile, e, src){
                    var o=linb(src).parent(2),w=o.width()+linb.dragDrop.getOffset().width;
                    o.width(w);
                    ...
                    profile.getSubNode(profile.keys.SCROLL).onScroll();
                    profile._limited=0;
                }
            },
            //row resizer
            ROWHANDLER:{
                ...
                onDragbegin:function(profile, e, src){
                    linb.dragDrop.proxyIn
                    .setStyle({
                        width:profile.getSubNode(profile.key).width()+'px',
                        height:'4px',
                        backgroundColor:'#ddd',
                        cursor:'n-resize'
                    });
                ...
                onDrag:function(profile, e, src){
                    var d=linb.dragDrop,b=0;
                    if(d.top<=d.limit_top || d.top>=d.limit_bottom)b=true;
                    if(b){
                        if(!profile._limited){
                            linb.dragDrop.proxyIn.setStyle('backgroundColor','#ff6600');
                            profile._limited=true;
                ...
                onDragend:function(profile, e, src){
                    var o=linb(src).parent(3),h=o.height()+linb.dragDrop.getOffset().height;
                    //for ie's strange bug
                    if(linb.browser.ie && h%2==1)h+=1;
                    o.height(h);
                    if(src.parentNode.id.startWith(profile.keys.HCELL0A)){
                        profile.box.resize(profile,null,profile.getSubNode(profile.keys.KEY).height());
                    }
                    profile._limited=0;
                }
            },
            ...
            HCELL:{
                onClick:function(profile, e, src){
                    ...
                    fun = me.fun||(me.fun = function(profile, root, index, type, order){
                        ...
                        if(root){
                            rows = root.sub;
                            parent = profile.getSubNode(profile.keys.SUB, root._serialId).get(0);
                        }else{
                            root={_created:true};
                            rows = profile.properties.rows;
                            parent = profile.getSubNode(profile.keys.BODY).get(0);
                        }
                        //sor sub first
                        var a1=[], a2=[], a3=[] ,a4=[];
                        ...
                        if(b){
                            ...
                            if(profile.properties.altRowsBg){
                                var altCls = profile.getClass(profile.keys.ALT);
                    ...
                    //show sort mark
                    profile.getSubNode(profile.keys.SORT, true).display('none');
                    var node = profile.getSubNode(profile.keys.SORT, col._serialId).display('block');
                    if(col._order = !col._order)
                        profile.addTagClass(node, profile.keys.SORT, '-checked');
                    else
                        profile.removeTagClass(node, profile.keys.SORT, '-checked');
                ...
                beforeMouseover:function(profile, e, src){
                    ...
                    profile.addTagClass(linb(src), profile.keys.HCELL, '-dragover');
                },
                beforeMouseout:function(profile, e, src){
                    if(false===profile.box._colDragCheck(profile,src))return;

                    linb.dragDrop._current=null;
                    profile.removeTagClass(linb(src), profile.keys.HCELL, '-dragover');
                },
                onDrop:function(profile, e, src){
                    ...
                    profile.getSubNode(profile.keys.HCELL, toId).addPre(linb.dragDrop.data);
                    //reposition cell dom nodes
                    _.each(toTh._cells, function(o,i){
                        profile.getSubNode(profile.keys.CELL,o).addPre(profile.getSubNode(profile.keys.CELL,fromTh._cells[i]));
                    });
            ...
            CELL0:{
                onClick:function(profile, e, src){
                   ...
                   if(p.disabled || row.disabled)return false;
                   if(row.sub)
                        profile.getSubNode(profile.keys.MARK, serialId).onClick(true);
                   linb(src).first().focus();
                   return false;
                },
                onMousedown:function(profile, e, src){
            ...
            CELL0A:{
                onFocus:function(profile, e, src){
                    if(profile.properties.activeMode=='row'){
                        var src = profile.getSubNode(profile.keys.CELLS, profile.getSubSerialId(src.id)).get(0);
            ...
            CELLS:{
                beforeMouseover:function(profile, e, src){
                    if(false===profile.box._rowDragCheck(profile,src))return;
                    linb.dragDrop._current=this;
                    profile.addTagClass(linb(src), profile.keys.CELLS, '-dragover');
                },
                beforeMouseout:function(profile, e, src){
                    if(false===profile.box._rowDragCheck(profile,src))return;

                    linb.dragDrop._current=null;
                    profile.removeTagClass(linb(src), profile.keys.CELLS, '-dragover');
                },
                onDrop:function(profile, e, src){
                    if(false===profile.box._rowDragCheck(profile,src))return;
                    profile.removeTagClass(linb(src), profile.keys.CELLS, '-dragover');
                ...
                onDblclick:function(profile, e, src){
                    ...
                    if(profile.onDblClickRow)profile.boxing().onDblClickRow(row,e,src);
                    return false;
                }
            },
            CELL:{
        ...
        DataModel:{
            ...
            headerHeight:{
                ini:20,
                action:function(v){
                    this.getSubNode(this.keys.HCELLS).height(v);
            ...
            rowHeight:{
                ini:20,
                action:function(v){
                    this.getSubNode(this.keys.CELLS, true).height(v);
                }
            },
            cellDfWidth: 80,
            ...
            colResizer:{
                ini:true,
                action:function(value){
                    this.getSubNode(this.keys.HCELLHANDLER,true).display(value?'':'none');
                }
            },
            rowHandler:{
                ini:true,
                action:function(value){
                    this.getSubNode(this.keys.HCELL0).display(value?'':'none');
                    this.getSubNode(this.keys.CELL0,true).display(value?'':'none');
                }
            },
            rowResizer:{
                ini:true,
                action:function(value){
                    this.getSubNode(this.keys.ROWHANDLER,true).display(value?'':'none');
                }
            },
            colSortable:{
                ini:true,
                action:function(value){
                    this.getSubNode(this.keys.SORT,true).display('none');
                }
            },
                ...
                action:function(value){
                    if(this.domNode){
                        ...
                        if(profile.$activeCell)
                            profile.removeTagClass(linb(profile.$activeCell), profile.keys.CELL, '-active');
                        if(profile.$activeRow)
                            profile.removeTagClass(linb(profile.$activeRow), profile.keys.CELLS, '-active');
                    }
                }
            }
        },
        EventHandlers:{
            ...
            onDblClickRow:function(row,e,src){}
        },
        ...
        _resetRowHandler : function(profile, row, baseW, subMargin, _layer){
            ...
            profile.getSubNode(profile.keys.CELL0, row._serialId).width(w);
            if(row.sub){
                ++_layer;
        ...
        _rowDrop:function(profile, src, flag){
            ...
            if(flag){
                ...
                if(toRow.sub)
                    ...
                    toParent = toRow.sub = [];
                    profile.getSubNode(profile.keys.MARK, toRow._serialId).display('block');
                }
                toIndex = -1;
            }else{
                toParentId = toRow._parent;
            ...
            if(profile.properties.rowHandler){
                ...
                if(toParentId!==fromParentId){
                    fromRow._parent=toParentId;
                    profile.box._resetRowHandler(profile, fromRow, profile.getSubNode(profile.keys.HCELL0).width(), profile.properties.$subMargin, _layer||0);
                }
            }
            ...
            if(flag)
                profile.getSubNode(profile.keys.SUB, toId).addLast(linb.dragDrop.data);
            else
                //move dom node
                profile.getSubNode(profile.keys.ROW, toId).addPre(linb.dragDrop.data);
        ...
        prepareItems:function(profile, arr, pid){
            var pro=profile.properties,
                ...
                rows=[],
                altCls = profile.getClass(profile.keys.ALT),
            ...
            for(var i=0,l=arr.length;i<l;i++){
                ...
                row.cells.each(function(g,j){
                    ...
                    n.cellCls =  this._clsCache[n.type] || (this._clsCache[n.type]=profile.getClass(profile.keys.CELL, '-' + n.type));

                    switch(n.type){
                        case 'checkbox':
                            n.checkboxCls = this._clsCache[n.type+':'+n.value] || (this._clsCache[n.type+':'+n.value]=profile.getClass(profile.keys.CHECKBOX, n.value?'-checked':''));
                            break;
                    }
        ...
        setSub:function(profile, item, flag){
            var id=profile.domId,
                ...
                markNode = profile.getSubNode(key1, serialId),
                subNode = profile.getSubNode(key2, serialId)
                ;
            ...
            if(item._checked){
                ...
                var openSub = function(profile, item, id, markNode, subNode, sub, flag){
                    var p = profile.properties;
                    //created
                    if(!item._created){
                        delete item.sub;
                    ...
                    if(!flag){
                        ...
                        subNode.fx({'height':[0,h]},function(){
                            subNode.setStyle({height:'0px',display:'block'});
                        },function(){subNode.setStyle({height:'auto'})}, 200, 8, 'outexp', profile.key+profile.id).start();
                    }else
                        subNode.display('block');

                    if(p.rowHandler){
                        var w1 = profile.getSubNode(profile.keys.HCELL0).width(),
                        w2 = (item._layer+2)*p.$subMargin;
                        if(w1<w2){
                            profile.getSubNode(profile.keys.HCELL0).width(w2);
                            profile.getSubNode(profile.keys.CELL0,true).widthBy(w2-w1);
                    ...
                    profile.addTagClass(markNode, key1, '-checked').addTagClass(markNode, key1, '-mouseover');
                ...
                var sub = item.sub;
                if(sub && sub.length){
                    openSub(profile, item, id, markNode, subNode, sub, flag);
                    if(flag){
                        sub.each(function(o){
                            if(o.sub && o.sub.length && !o.iniFold && !o._checked)
        ...
        changeCellValue:function(profile, cellId, value){
            ...
            if(cell.value!=value){
                ...
                switch(type){
                    case 'checkbox':
                        if(value)
                            profile.addTagClass(profile.getSubNode(profile.keys.CHECKBOX, cellId),profile.keys.CHECKBOX, '-checked');
                        else
                            profile.removeTagClass(profile.getSubNode(profile.keys.CHECKBOX, cellId), profile.keys.CHECKBOX,'-checked');
                        break;
                    case 'number':
                    ...
                    case 'colorpicker':
                    default:
                        profile.getSubNode(profile.keys.CELLA, cellId).text(render?render(value):value);
                }
        ...
        activeCell:function(profile, id){
            ...
            if(profile.$activeCell)
                profile.removeTagClass(linb(profile.$activeCell), profile.keys.CELL, '-active');
            profile.addTagClass(linb(id), profile.keys.CELL, '-active');
        ...
        activeRow:function(profile, id){
            ...
            if(profile.$activeRow)
                profile.removeTagClass(linb(profile.$activeRow), profile.keys.CELLS, '-active');
            profile.addTagClass(linb(id), profile.keys.CELLS, '-active');
        ...
        editCell:function(profile, cellId){
            ...
            var cellNode = profile.getSubNode(profile.keys.CELL, cellId),
                baseNode = profile.getSubNode(profile.keys.SCROLL),
...
//                        linb.SC('linb.UI.CheckBox');
                ...
                switch(type.split(':')[0]){
                    ...
                    case 'popbox':
                        widget =  new linb.UI.ComboInput;
                        widget.setType(type).onCustomPop(function(pro){
                            var cell=pro.$cell;
                            //event
...
//                        widget =  linb.create('CheckBox');
            ...
            .beforeNextFocus(function(pro, key, shift, e){
                ...
                profile.getSubNode(profile.keys.CELLA, cell._serialId).onKeydown(true,hash);
                //prevent
                return false;
            });
            ...
            if(format){
                ...
                t = profile.box.getCellPro(profile, cell,'validTips');
                if(t && widget.setValidTips)widget.setValidTips(t);
            }
        ...
        resize:function(profile,w,h){
            profile.getSubNode(profile.keys.BORDER).cssSize({ width :w, height :h});
            var rh;
            if(h)
                rh = profile.getSubNode(profile.keys.HEADER).realHeight();
            profile.getSubNode(profile.keys.BOX).cssSize({width: w?w:null, height:h?h:null});
            profile.getSubNode(profile.keys.SCROLL).setRegion({top:rh?rh:null, width:w?w:null, height: h?(h-rh):null}).onScroll();
        }
   }
});