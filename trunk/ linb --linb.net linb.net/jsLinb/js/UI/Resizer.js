...
Class("linb.UI.Resizer","linb.UI",{
    Instance:{
        ...
        rePosSize:function(){
            this.each(function(o){
                var t1=o.getSubNode(o.keys.KEY), t2=o._target;
                if(!t2 || t2.isEmpty())return;
                if(!o.properties.child){
                     o.region=o.boxing().getRegion();
                        t1
                        .cssPos({left :o.region.l, top :o.region.t})
                        .offsetWidth(o.region.r-o.region.l)
                        .offsetHeight(o.region.b-o.region.t);
                }
                if(!o.regPool)o.regPool=linb([],false);
                if(o.regions){
                    o.regPool.add(o.regions);
                    o.regions.display('none');
                }
                o.regions=linb([],false);

                if(o.regionBlocks){
                    var t,fun=function(){
                        var t = o.boxing().getTarget();
                        var key = linb.event.currentKey;
                        if(t.length()>1){
                            if(key && key[2]){
                                t.minus(linb(this.tid));
                                o.boxing().resetTarget(t);
                            }else{
                                o.boxing().focus(this.tid);
                            }
                        }
                    };
                    ...
                    o.getSubNode(o.keys.KEY).addLast(o.regions.display('block'));
                }
            });
            this.focus();
            return this;
        ...
        resetTarget:function(target,flag){
            ...
            if(target){
                ...
                this.rePosSize();
                this.reBoxing().zIndex(linb.ini.top_zIndex).display('block');
            }else{
                profile._target = linb([],false);
                this.reBoxing().zIndex(0).display('none');
            }
            if(target){
                ids=[];
        ...
        focus:function(id){
           ...
           if(index!=-1){
                profile.regions.border('dashed 1px blue');
                linb([profile.regions.get(index)],false).border('solid 1px red');
        ...
        active:function(flag){
            return this.each(function(profile){
                profile.getSubNode(profile.keys.MOVE).setStyle('backgroundPosition','-17px top');
                var o = profile.getSubNode(profile.keys.LT).add(
                    profile.getSubNode(profile.keys.T)
                ).add(
                    profile.getSubNode(profile.keys.RT)
                ).add(
                    profile.getSubNode(profile.keys.R)
                ).add(
                    profile.getSubNode(profile.keys.RB)
                ).add(
                    profile.getSubNode(profile.keys.B)
                ).add(
                    profile.getSubNode(profile.keys.LB)
                ).add(
                    profile.getSubNode(profile.keys.L)
                );
                if(linb.browser.ie)
                    o.background('url('+linb.ini.path+'bg.gif)');
                else
        ...
        inActive:function(){
            return this.each(function(profile){
                ...
                profile.getSubNode(profile.keys.MOVE).setStyle('backgroundPosition','-17px -17px');
                var o = profile.getSubNode(profile.keys.LT).add(
                    profile.getSubNode(profile.keys.T)
                ).add(
                    profile.getSubNode(profile.keys.RT)
    ...
    Initialize:function(){
        ...
        _.each({
            ...
            resizable:function(properties, target, onUpdate){
                ...
                var r = new linb.UI.Resizer(properties).attachTo(target, linb([o],false));
                //set event
                if(onUpdate)
                    r.onUpdate(onUpdate);
                return r;
    ...
    Static:{
        Templates:{'default':{
            tagName:'div',
            style:'{width}{height}{left}{top}{position};display:{display};visibility:{visibility};',
            TAG:{}
        }},
        Appearances:{'default':{
            KEY:{
        ...
        dynamicTemplate:function(profile){
            ...
            var map= arguments.callee.map || (arguments.callee.map={
                ...
                T:{tagName:'div', style:'top:-{edge}px;margin-left:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                RT:{tagName:'div', style:'top:-{edge}px;right:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                R:{tagName:'div', style:'right:-{edge}px;margin-top:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                RB:{tagName:'div', style:'bottom:-{edge}px;right:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                B:{tagName:'div', style:'bottom:-{edge}px;margin-left:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                LB:{tagName:'div',style:'bottom:-{edge}px;left:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                L:{tagName:'div', style:'left:-{edge}px;margin-top:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                LT:{tagName:'div', style:'left:-{edge}px;top:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                cover:{
                    T:{tagName:'div', style:'width:100%;left:0;top:-{edge}px;height:{handlerSize}px;'},
                    RT:{tagName:'div', style:'top:-{edge}px;right:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                    R:{tagName:'div', style:'height:100%;top:0;right:-{edge}px;width:{handlerSize}px;' },
                    RB:{tagName:'div', style:'right:-{edge}px;bottom:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                    B:{tagName:'div', style:'width:100%;left:0;bottom:-{edge}px;height:{handlerSize}px;'},
                    LB:{tagName:'div', style:'left:-{edge}px;bottom:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'},
                    L:{tagName:'div', style:'height:100%;top:0;left:-{edge}px;width:{handlerSize}px;' },
                    LT:{tagName:'div', style:'top:-{edge}px;left:-{edge}px;width:{handlerSize}px;height:{handlerSize}px;'}
                }
            });
            ...
            */
            ...
            if(!template){
                ...
                n = profile.getClass(profile.key, '-handler') + " ";
                ...
                // if hidden
                if(!pro.visible){
                    n = profile.getClass(profile.key, '-hidden') + " ";
                    if(t=template.T)t.className += n;
                    if(t=template.RT)t.className += n;
                    if(t=template.R)t.className += n;
                    if(t=template.RB)t.className += n;
        ...
        prepareData:function(profile){
            ...
            // handlerSize
            t.handlerSize = parseInt(t.offset)*2;
            // reset offset value
        ...
        onMousedown:function(profile, e, src, ddparas){
            ...
            if(ck && (ck[1] || (linb.browser.kde&&ck[0]=='space'))){
                ...
                if(profile.properties.child){
                    ...
                    o = profile.getSubNode(profile.keys.KEY);
                    var absPos = o.absPos();
                    pos=o.cssPos();
                    posbak=_.copy(pos);
                    size=o.cssSize();
        ...
        onDragbegin:function(profile, e){
            if(profile.properties.child){
                ...
                //custom proxy
                profile.proxy = linb.dom.getTemp();
                profile.proxy
                .html(' ',false)
        ...
        onDrag:function(profile, e,src, ddparas){
            ...
            if(os.width == profile.oos.width && os.height == profile.oos.height)return;
            profile.oos=os;
            ...
            if(o.left){
                // width of proxy
                w = profile.o_w - os.width;
                // left of proxy
                x = profile.o_pos.left + os.width;
                if(w<t.minWidth){
                    w=t.minWidth;
            ...
            if(o.top){
                h = profile.o_h - os.height;
                y = profile.o_pos.top + os.height;
                if(h<t.minHeight){
                    h=t.minHeight;
