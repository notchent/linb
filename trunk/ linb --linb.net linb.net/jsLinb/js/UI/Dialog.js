Class("linb.UI.Dialog",["linb.UI.Widget","linb.UI.iContainer"],{
    Instance:{
        html:function(html){
            var pro=this.get(0);
            pro.getSubNode(pro.keys.PANEL).html(html);
        },
        ...
        getPanelPara:function(){
            var para = _.copy(this.get(0).properties);
            return para;
        },
    ...
    Static:{
        ...
        Appearances:{'default':{
            ...
            TITLE:{
                ...
                background: linb.UI.getCSSImgPara('title.gif', ' left top repeat-x'),
                '*font-size':'0',
                '*line-height':'0'
            },
            'TITLE-focus':{
            ...
            'MIN, RESTORE, MAX, PIN, CLOSE, LAND':{
                position:'relative',
        ...
        Behaviors:{'default':{
            _hoverEffect:{PIN:'PIN',MIN:'MIN',MAX:'MAX',RESTORE:'RESTORE',CLOSE:'CLOSE',LAND:'LAND'},
            _clickEffect:{PIN:'PIN',MIN:'MIN',MAX:'MAX',RESTORE:'RESTORE',CLOSE:'CLOSE',LAND:'LAND'},
            onMousedown:function(profile, e){
                profile.box.active(profile);
            },

            onDragend:function(profile){
                var pos = profile.root.cssPos();
                profile.properties.left = pos.left;
                profile.properties.top = pos.top;
            },
            TITLE:{
            ...
            PIN:{
                onClick:function(profile, e, src){
                    ...
                    //set appea
                    if(t.pinned)
                        profile.addTagClass(profile.getSubNode(key), key,'-checked', true);
                    else
        ...
        DataModel:{
            ...
            minBtn:{
                ini:true,
                action:function(v){
                    var o = this.getSubNode(this.keys.MIN);
                    if(v)
                        o.inlineBlock();
                    else
                        o.display('none');
            ...
            maxBtn:{
                ini:true,
                action:function(v){
                    var o = this.getSubNode(this.keys.MAX);
                    if(v)
                        o.inlineBlock();
                    else
                        o.display('none');
            ...
            closeBtn:{
                ini:true,
                action:function(v){
                    var o = this.getSubNode(this.keys.CLOSE);
                    if(v)
                        o.inlineBlock();
                    else
                        o.display('none');
            ...
            pinBtn:{
                ini:true,
                action:function(v){
                    var o = this.getSubNode(this.keys.PIN);
                    if(v)
                        o.inlineBlock();
                    else
                        o.display('none');
            ...
            landBtn:{
                ini:false,
                action:function(v){
                    var o = this.getSubNode(this.keys.LAND);
                    if(v)
                        o.inlineBlock();
                    else
                        o.display('none');
            ...
            status:{
                ...
                action:function(v,ov){
                    switch(v){
                        case 'min':
                            this.box.min(this,ov);
                            break;
                        case 'max':
                            this.box.max(this,ov);
                            break;
                        default:
                            this.box.restore(this,ov);
                    }
                }
            }
        },
        ...
        createdTrigger:function(){
            this.destroyTrigger = function(){
                if(this.$inModal)this.box.unModal(this);
            };
        },
        envTrigger:function(){
            ...
            if(t.status=='min')
                ...
                this.box.resize(this, t.width, t.height);
        },
        prepareData:function(profile){
            arguments.callee.upper.call(this, profile);

            var data = profile.data;
            data.minDisplay = data.minBtn?'':'display:none';
            data.maxDisplay = data.maxBtn?'':'display:none';
            data.closeDisplay = data.closeBtn?'':'display:none';
            data.pinDisplay = data.pinBtn?'':'display:none';
            data.landDisplay = data.landBtn?'':'display:none';

            data.iconDisplay = data.icon?'':'display:none';
        ...
        min:function(profile, ov){
            ...
            // hide those
            profile.getSubNode(profile.keys.PANEL).display('none');
            profile.getSubNode(profile.keys.STATUS).display('none');

            if(t.minBtn){
                // show restore button
                profile.getSubNode(profile.keys.RESTORE).inlineBlock();
                // hide min button
                profile.getSubNode(profile.keys.MIN).display('none');
            }
            ...
            var h1=o.height(),
                h2=profile.getSubNode(profile.keys.BORDER).height(),
                h=profile.getSubNode(profile.keys.TITLE).height();
            // resize
            o.cssSize({ width :t.minWidth, height :h+h1-h2},true);
        },
        max:function(profile, ov){
            var o = profile.boxing().reBoxing(),p=o.parent();
            ...
            // hide pin button
            if(t.pinBtn)
                profile.getSubNode(profile.keys.PIN).display('none');
            if(t.maxBtn){
                // hide max button
                profile.getSubNode(profile.keys.MAX).display('none');
                // show restore button
                profile.getSubNode(profile.keys.RESTORE).inlineBlock();
            }
        ...
        restore:function(profile, ov){
            ...
            // hide restore button
            profile.getSubNode(profile.keys.RESTORE).display('none');

            t.status='normal';
        },
        unMax:function(profile){
            var t=profile.properties;
            profile.getSubNode(profile.keys.MAX).inlineBlock();
            if(t.pinBtn)
                profile.getSubNode(profile.keys.PIN).inlineBlock();
            ...
            // resize
            profile.box.resize(profile, profile.properties.width, profile.properties.height);
        },
        unMin:function(profile){
            var t=profile.properties;
            profile.getSubNode(profile.keys.PANEL).display('block');
            profile.getSubNode(profile.keys.STATUS).display('block');
            profile.getSubNode(profile.keys.MIN).inlineBlock();

            // add/remove resize
            if(t.resizable && !t.pinned &&profile.$resizer)
                    profile.$resizer.show();

            profile.root.cssSize({width:profile.properties.width, height:profile.properties.height});
            // resize
            profile.box.resize(profile, profile.properties.width, profile.properties.height);
        },
        active:function(profile){
            if(this.activeWndId == profile.$id)return;

            this.deActive();

            var o=linb(this.activeWndId = profile.domId);
            //in ie, .children can't get the same thread added node(modal div,  here)
            var t1=o.topZindex(), t2=o.zIndex();
            o.zIndex(t1>t2?t1:t2);

            profile.addTagClass(profile.getSubNode(profile.keys.TITLE), profile.keys.TITLE,'-focus');
            profile.addTagClass(profile.getSubNode(profile.keys.CMDS), profile.keys.CMDS,'-focus');
            this.activeWndId = profile.$id;
        },
        deActive:function(profile){
            var id,profile;
            if((id=linb.UI._cache[this.activeWndId]) && linb.dom.byId(id)){
                profile = linb(id).reBoxing(this.KEY).get(0);
                profile.removeTagClass(profile.getSubNode(profile.keys.TITLE), profile.keys.TITLE,'-focus');
                profile.removeTagClass(profile.getSubNode(profile.keys.CMDS), profile.keys.CMDS,'-focus');
            }
        ...
        modal:function(profile){
            ...
            if(!p.isEmpty()){
                if(!profile.$inModal){
                    ...
                    linb.event.focusHook.push(profile.domNode);
        ...
        unModal:function(profile){
            if(profile.$inModal){
                ...
                profile.$modalDiv.reBoxing().display('none').addToLast(profile.getSubNode(profile.keys.BORDER));
        ...
        _adjust:function(dialog,caption, content){
            ...
            // to none for get correct size
            node.cssSize({ width :'auto', height :'auto'}).overflow('visible').html(content);
            var size = node.cssSize();
        ...
        resize:function(profile,w,h){
            var size = arguments.callee.upper.apply(this,arguments),region={};
            var v1=profile.getSubNode(profile.keys.TITLE),
                v2=profile.getSubNode(profile.keys.PANEL),
                v3=profile.getSubNode(profile.keys.STATUS);
            if(h){
                var bh = size.height,
