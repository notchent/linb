Class("linb.UI.Layout",["linb.UI.iList", "linb.UI.iContainer"],{
    Instance:{
        getPanel:function(id){
            var o=this.get(0);
            return o.getSubNode(o.keys.PANEL, o.getSubSerialIdByItemId(id));
        },
        attach:function(ui, id){
            //first
            this.appendChild(ui, id||'main');
    ...
    Static:{
            ...
            MOVE:{
                onMousedown:function(profile, e, src){
                    ...
                    var main = profile.getItemByItemId('main'),
                        ...
                        o=profile.getSubNode(profile.keys.ITEM, itemId),
                        m=profile.getSubNode(profile.keys.ITEM, profile.getSubSerialIdByItemId('main')),
                        cursor=o.cursor(),
                        h,w,mh,mw,offset1,offset2;

                    var t=profile.properties,cursor=linb([src],false).cursor();
                ...
                onDrag:function(profile, e, src){
                    var t=profile.properties,d=linb.dragDrop,b=0;
                    if(t.type=='vertical'){
                ...
                onDragend:function(profile, e, src){
                    var t=profile.properties,o=linb([src],false).parent(),r=profile.getSubNode(profile.key);
                    var item = profile.getItemByDom(src);

                    //add offset and refresh
                    if(t.type=='vertical'){
                        //use size to ignore onresize event once
                        o.height(item.size =  profile._cur + (profile.pos=='before'?1:-1)*linb.dragDrop.getOffset().height);
                        profile.box.resize(profile,null,r.height());
                    }else{
                        o.width(item.size = profile._cur + (profile.pos=='before'?1:-1)*linb.dragDrop.getOffset().width);
                        //use size to ignore onresize event once
            ...
            CMD:{
                onMousedown:function(profile, e, src){
                    var t=profile.properties,
                        ...
                        itemId = profile.getSubSerialId(id),
                        item = profile.getItemByDom(src),
                        r=profile.getSubNode(profile.key),
                        main = profile.getItemByItemId('main'),
                        m=profile.getSubNode(profile.keys.ITEM, profile.getSubSerialIdByItemId('main'));

                    var o = profile.getSubNode(profile.keys.ITEM,itemId),
                        panel = profile.getSubNode(profile.keys.PANEL,itemId),
                        move = profile.getSubNode(profile.keys.MOVE,itemId);
                    ...
                    if(t.type=='vertical'){
                        ...
                        if(item.hide){
                            if(item.size <= m.height() - main.min + t._handlerSize){
                                ...
                                profile.removeTagClass(move, profile.keys.MOVE,'-checked');
                            }else
                        ...
                        // to min and fix mode
                            ...
                            if(!item.locked)
                                move.cursor('default');
                            profile.addTagClass(move, profile.keys.MOVE,'-checked');
                        }
                        profile.box.resize(profile,null,r.height());
                    }else{
                        if(item.hide){
        ...
        DataModel:{
            ...
            type:{
                ...
                action:function(value, ovalue){
                    if(value != ovalue){
                        var
                        nodes2 = this.getSubNode(this.keys.ITEM,true),
                        nodes1 = this.getSubNode(this.keys.MOVE,true),
                        nodes3 = this.getSubNode(this.keys.CMD,true);
                        ...
                        var size = this.root.cssSize();
                        this.box.resize(this, size.width, size.height);
                    }
                }
            },
            dock:'fill',
        ...
        prepareItem:function(profile, item){
            if(item.id=='main'){
                item.cls1=profile.getClass(profile.keys.ITEM, '-main');
                item.cls2  = profile.getClass(profile.keys.MOVE, '-main');
                item.cls3  = profile.getClass(profile.keys.CMD, '-main' );
                return;
            }
            ...
            item.cls1  = profile.getClass(profile.keys.ITEM, '-' + pos );
            item.cls2  = profile.getClass(profile.keys.MOVE, '-' + pos );
            item.cls3  = profile.getClass(profile.keys.CMD, '-' + pos );
            ...
            profile.properties.items.each(function(item){
                if(item.id!='main'){
                    if(item.hide && (t=profile.getSubSerialIdByItemId(item.id))){
                            item.hide=false;
                            profile.getSubNode(profile.keys.CMD,t).onMousedown();
                        }
                }
            });
        },
        resize:function(profile,w,h){
            ...
            t.items.each(function(o){
                ...
                obj[itemId] = linb.dom.iniBox();
                obj2[itemId] = linb.dom.iniBox();
            });
            if(t.type!='vertical'){
                if(!_.isNull(w)){
                    //get left
                    ...
                    t.items.each(function(o){
                        ...
                        itemId = profile.getSubSerialIdByItemId(o.id);
                        if(o.pos=='before'){
                            n=profile.getSubNode(key, itemId);
            ...
            _.each(obj2, function(o, id){
                profile.getSubNode(key, id).setRegion(o);
            });
        }
    }
});
