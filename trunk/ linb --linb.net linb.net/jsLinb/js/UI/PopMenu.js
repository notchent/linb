Class("linb.UI.PopMenu",["linb.UI.Widget","linb.UI.iList","linb.UI.iNavigator"],{
    Instance:{
        ...
        setScrollButtons:function(){
            return this.each(function(profile){
                var
                o=profile.getSubNode(profile.keys.ITEMS),
                t=o.offsetTop(),
                h=o.offsetHeight(),
                b = profile.getSubNode(profile.keys.BORDER),
                hh=b.offsetHeight();

                if(t === 0)
                    profile.getSubNode(profile.keys.TOP).display('none');
                else
                    profile.getSubNode(profile.keys.TOP).display('block');

                if(hh>h+t)
                    profile.getSubNode(profile.keys.BOTTOM).display('none');
                else
                    profile.getSubNode(profile.keys.BOTTOM).display('block');
            })
        },
        scrollToBottom:function(){
            return this.each(function(profile){
                var o = profile.getSubNode(profile.keys.ITEMS),
                border = profile.getSubNode(profile.keys.BORDER),
                y = o.offsetTop(),
                offset,
                h = o.offsetHeight(),
        ...
        pop:function(obj, parent, type){
            ...
            //ensure created
            if(!profile.created)profile.boxing().create();

            //clear highLight first
            if(profile.$highLight)
                profile.removeTagClass(linb([profile.$highLight],false), profile.keys.ITEM, '-mouseover');
...
/*
            ...
            if(t && t.length){
                var node = profile.getSubNode(profile.keys.ITEM, profile.getSubSerialIdByItemId(t[0].id)).get(0);
...
*/
    ...
    Static:{
            ...
            ITEM:{
                onMouseover:function(profile, e, src){
                    ...
                    if(!Cancel){
                        ...
                        profile.addTagClass(linb([profile.$highLight],false), profile.keys.ITEM, '-mouseover');
                    ...
                    if(!Cancel && item.sub){
                        ...
                        if(!(pop = profile.$allRelatedPopMenus[itemId])){
                            pop = linb.create('PopMenu',{position:'absolute', items:item.sub});
                            pop.onMenuSelected(function(pro, id, src){
                                profile.boxing().onMenuSelected(profile, id, src);
                            });
                            //set pool to parent
                            pop.get(0).$hideMenuPool = profile.$hideMenuPool || profile.getSubNode(profile.keys.POOL);
                ...
                onMouseout:function(profile, e, src){
                    ...
                    if(profile.$subPopMenuShowed){
                        ...
                        profile.removeTagClass(linb([src],false), profile.keys.ITEM, '-mouseover');
                        profile.$highLight = null;
                ...
                onClick:function(profile, e, src){
                    ...
                    if(item.sub){
                        ...
                        profile.removeTagClass(linb(this),profile.keys.ITEM,'-mouseover');
                ...
                onFocus:function(profile, e, src){
                    var box = profile.getSubNode(profile.keys.BOX),
                ...
                onKeydown : function(profile, e, src){
                    ...
                    switch(key){
                        case 'up':
                            ...
                            node = profile.getSubNode(profile.keys.ITEM, profile.getSubSerialIdByItemId(tid)).get(0);
                            if(node)node.focus();
                            break;
                        case 'down':
                            r=true;
        ...
        resize:function(profile,w,h){
            ...
            /*
            var v=profile.getSubNode(profile.keys.BOX),
            o = profile.getSubNode(profile.keys.BORDER);
            if(!_.isNull(w)){
                v.width(o.width());
            }
            if(!_.isNull(h)){
