...
/*        attach:function(ui){
            ...
            this.each(function(v){
                var c=v.getSubNode('GROUP');
                c.attach(ui);
            });
            return this;
        },*/
        updateItem:function(id, txt){
            this.get(0).getSubNodeByItemId('CAPTION', id).html(txt,false);
        },
        hideItem:function(id){
            this.get(0).getSubNodeByItemId('ITEM', id).display('none');
        },
        disableGroup:function(id, value){
            return this.each(function(profile){
                profile.getSubNodeByItemId('GROUP', id).opacity(value?0.5:1);
    ...
    Static:{
        ...
        DataModel:{
            ...
            tabindex:{
                action:function(value){
                    if(this.domNode)
                        this.getSubNode('ITEM',false).tabIndex(value);
                }
            },
            ...
            handler:{
                ini:true,
                action:function(v){
                    this.getSubNode('HANDLER',true).display(v?'':'none');
                }
            },
            position:'absolute',
            align:{
                ...
                listbox:['left','right'],
                action:function(v){
                    this.getSubNode('ITEMS', true).setStyle('textAlign', v);
                }
            },
            dock:{
                ini:'top',
        ...
        prepareItem:function(profile, item, sitem){
            ...
            a.each(function(item){
                ...
                t.typeCls = profile.getClass('ITEM', '-'+t.type);
